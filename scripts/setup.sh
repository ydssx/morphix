...
helm upgrade --install loki --namespace=loki-stack grafana/loki-stack -f deploy/kubernetes/helm/loki.yaml --create-namespace
...
helm upgrade --install metrics-server metrics-server/metrics-server -f deploy/kubernetes/helm/metrics-server.yaml
helm upgrade --install kubernetes-dashboard kubernetes-dashboard/kubernetes-dashboard -f deploy/kubernetes/helm/dashboard.yaml --create-namespace --namespace kubernetes-dashboard